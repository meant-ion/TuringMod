...
import { OutgoingMessageTypes, WebSocketOpCode, OutgoingMessage, OBSEventTypes, IncomingMessage, IncomingMessageTypes, OBSRequestTypes, OBSResponseTypes } from './types.js';
export declare const debug: createDebug.Debugger;
export declare class OBSWebSocketError extends Error {
    code: number;
    constructor(code: number, message: string);
}
export declare type EventTypes = Merge<{
...
declare type MapValueToArgsArray<T extends Record<string, unknown>> = {
    [K in keyof T]: T[K] extends void ? [] : [T[K]];
};
declare type IdentificationInput = SetOptional<Except<OutgoingMessageTypes[WebSocketOpCode.Identify], 'authentication'>, 'rpcVersion'>;
declare type HelloIdentifiedMerged = Merge<Exclude<IncomingMessageTypes[WebSocketOpCode.Hello], 'authenticate'>, IncomingMessageTypes[WebSocketOpCode.Identified]>;
export declare abstract class BaseOBSWebSocket extends EventEmitter<MapValueToArgsArray<EventTypes>> {
     ...
     * @param e Event
     */
    protected onMessage(e: MessageEvent): Promise<void>;
    /**
